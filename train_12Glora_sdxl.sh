...
max_train_epoches=12 # max train epoches | 最大训练 epoch
...
algo="lokr" # algo参数，指定训练lycoris模型种类，包括lora(就是locon)、loha、IA3以及lokr、dylora、full(DreamBooth先训练然后导出lora) ，6个可选
...
output_name="112101lora" # output model name | 模型保存名称
...
if [[ "$train_mode" == "sdxl"* ]]; then
  launch_script="sdxl_$launch_script"
...
elif [[ "$dataset_config" ]]; then
  ext_args+=( "--dataset_config=$dataset_config" )
else
...
if [[ "$persistent_workers" -ne 0 ]]; then
...
elif [[ "$multires_noise_iterations" ]]; then
...
if [[ "$network_dropout" -ne 0 ]]; then
  ...
  if [[ "$enable_dylora" -ne 0 ]]; then
    ...
    if [[ "$rank_dropout" ]]; then
      ext_args+=( "rank_dropout=$rank_dropout" )
    fi
    if [[ "$module_dropout" ]]; then
...
if [[ "$lr_scheduler_num_cycles" -ne 0 ]]; then
  ext_args+=( "--lr_scheduler_num_cycles=$lr_scheduler_num_cycles" )
fi

if [[ "$base_weights" ]]; then
  ext_args+=( "--base_weights" )
  IFS=' ' read -ra base_weights_array <<< "$base_weights"
  for base_weight in "${base_weights_array[@]}"; do
    ext_args+=( "$base_weight" )
  done
  ext_args+=( "--base_weights_multiplier" )
  IFS=' ' read -ra ratio_array <<< "$base_weights_multiplier"
  for ratio in "${ratio_array[@]}"; do
    ext_args+=( "$(echo "$ratio" | bc -l)" )
  done
fi
