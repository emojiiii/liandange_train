...
$output_name = "112102lora" # output model name | 模型保存名称
...
if [[ $v_parameterization ]]; then
    ext_args+=("--v_parameterization")
fi

if [[ $scale_v_pred_loss_like_noise_pred ]]; then
...
if [[ $shuffle_caption ]]; then
    ext_args+=("--shuffle_caption")
fi

if [[ $weighted_captions ]]; then
...
elif [[ $enable_lora_fa ]]; then
    network_module="networks.lora_fa"
elif [[ $enable_oft ]]; then
