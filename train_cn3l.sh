...
# Train Model | 训练模式
model="sdxl_cn3l" #lora、db、sdxl_lora、sdxl_db、contralnet(unfinished)

# Train data path | 设置训练用模型、图片
pretrained_model="./Stable-diffusion/SDXL/infinianimexl_v16.safetensors" # base model path | 底模路径
is_v2_model=0                             # SD2.0 model | SD2.0模型 2.0模型下 clip_skip 默认无效
v_parameterization=0 # parameterization | 参数化 v2 非512基础分辨率版本必须使用。
vae="./VAE/sdxl_vae.safetensors"
train_data_dir="./train/dwpose/train"              # train dataset path | 训练数据集路径
...
save_every_n_epochs=2 # sa2ve every n epochs | 每 N 个 epoch 保存一次

gradient_checkpointing=1 #梯度检查，开启后可节约显存，但是速度变慢
gradient_accumulation_steps=0 # 梯度累加数量，变相放大batchsize的倍数

network_dim=256   # network dim | 常用 4~128，不是越大越好
...
output_name="tile"           # output model name | 模型保存名称
...
enable_bucket=0 # arb for diff wh | 分桶
...
#cn3l
conditioning_data_dir="./train/tile/tile3"
cond_emb_dim=128
...
if [ $save_state -ne 0 ]; then extArgs+=("--save_state"); fi
...
elif [ $enable_dylora_train -ne 0 ]; then
...
if [[ $min_snr_gamma -ne 0 ]]; then extArgs+=("--min_snr_gamma=$min_snr_gamma"); 
elif [[ $debiased_estimation_loss -ne 0 ]]; then extArgs+=("--debiased_estimation_loss"); 
fi

if [[ $ip_noise_gamma != "0" ]]; then extArgs+=("--ip_noise_gamma=$ip_noise_gamma"); fi
