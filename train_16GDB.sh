...
$pretrained_model = "D:\sd-webui-aki-v4.4\models\Stable-diffusion\sd_xl_base_1.0.safetensors" # base model path | 底模路径
$vae = "D:\sd-webui-aki-v4.4\models\VAE\sdxl-vae-fp16-fix.safetensors"
$is_v2_model = 0 # SD2.0 model | SD2.0模型 2.0模型下 clip_skip 默认无效
$v_parameterization = 0 # parameterization | 参数化 v2 非512基础分辨率版本必须使用。
$train_data_dir = "C:\Users\Administrator\Desktop\3dstyle" # train dataset path | 训练数据集路径
$reg_data_dir = ""	# reg dataset path | 正则数据集化路径
$network_weights = "" # pretrained weights for LoRA network | 若需要从已有的 LoRA 模型上继续训练，请填写 LoRA 模型路径。
$training_comment = "3dstyle" # training_comment | 训练介绍，可以写作者名或者使用触发关键词
$dataset_class = ""
...
$resolution = "1024,1536" # image resolution w,h. 图片分辨率，宽,高。支持非正方形，但必须是 64 倍数。
...
$output_name = "1121033dstyle" # output model name | 模型保存名称
$save_model_as = "safetensors" # model save ext | 模型保存格式 ckpt, pt, safetensors
$mixed_precision = "fp16" # bf16效果更好，默认fp16
$save_precision="fp16" # bf16效果更好，默认fp16
$full_fp16 = 1 #开启全fp16模式，自动混合精度变为fp16，更节约显存，实验性功能
...
if ($train_mode -ilike "sdxl*"){
  ...
  if ($no_half_vae -ne 0) { 
    ...
    if [ -n "$is_v2_model" ]; then
        ext_args+=("--v2")
        min_snr_gamma=0
        debiased_estimation_loss=0
        if [ -n "$v_parameterization" ]; then
    ...
    if [ -n "$prior_loss_weight" ] && [ "$prior_loss_weight" -ne 1 ]; then
        ext_args+=("--prior_loss_weight=$prior_loss_weight")
    fi

    if [ -n "$network_dim" ]; then
        ext_args+=("--network_dim=$network_dim")
    fi

    if [ -n "$network_alpha" ]; then
        ext_args+=("--network_alpha=$network_alpha")
    fi

    if [ -n "$training_comment" ]; then
    ...
    if [ -n "$shuffle_caption" ]; then
        ext_args+=("--shuffle_caption")
    fi

    if [ -n "$weighted_captions" ]; then
        ext_args+=("--weighted_captions")
    fi

    if [ -n "$cache_latents" ]; then
        ext_args+=("--cache_latents")
        if [ -n "$cache_latents_to_disk" ]; then
    ...
    if [ -n "$noise_offset" ]; then
        ext_args+=("--noise_offset=$noise_offset")
        if [ -n "$adaptive_noise_scale" ]; then
            ext_args+=("--adaptive_noise_scale=$adaptive_noise_scale")
        fi
    elif [ -n "$multires_noise_iterations" ]; then
    ...
    if [ "$network_dropout" -ne 0 ]; then
        ...
        if [ "$enable_dylora" -ne 0 ]; then
            ...
            if [ -n "$rank_dropout" ]; then
                ext_args+=("rank_dropout=$rank_dropout")
            fi
            if [ -n "$module_dropout" ]; then
                ext_args+=("module_dropout=$module_dropout")
            fi
        fi
    fi

    if [ -n "$enable_block_weights" ]; then
        ext_args+=("--network_args")
    ...
    if [ -n "$lr_scheduler_num_cycles" ]; then
    ...
    if [ -n "$base_weights" ]; then
        ...
        IFS=' ' read -ra base_weights_multiplier_arr <<< "$base_weights_multiplier"
        for ratio in "${base_weights_multiplier_arr[@]}"; do
            ext_args+=([float]$ratio)
        done
    fi

    if [ -n "$enable_bucket" ]; then
